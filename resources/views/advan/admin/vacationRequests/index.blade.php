...
<div class="container">
    <div class="card card-custom gutter-b">
        ...
        <div class="card-body py-0">
            <!--begin::Table-->
            <div class="vacation-requests-table-body">

               @includeIf('advan.admin.vacationRequests.table-data')
...
<script>
 function load_data_table(page = '') {
        $.ajax({
            url: '{{url("admin/vacation-requests/")}}?page=' + page,
            data: {
                search: $('#search_input').val()
            },
            type: "get",
            success: function(response) {
                $('.vacation-requests-table-body').html(response.vacationRequests)
...
$(document).on('click','.delete-vacation',function(){
            var id = $(this).data('vacation-id');
            Swal.fire({
                title: 'هل أنت متأكد من حذف طلب الاجازة',
                showDenyButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{route("admin.vacation-requests.delete")}}' ,
                        type: "POST",
                        data: {id:id},
                        success: function( response ) {
